...
- **Architecture invariants hold** (see `CLAUDE.md`): renderer never touches SQLite/Node/fs (only
...
- Commit per component: subject `harden(<area>): <summary>`, body, trailer `Co-Authored-By: Claude
...
function auditPrompt(c, round, fixReport) { return [
  ...
  'Scope: ' + c.scope + '\nCode: ' + c.paths + '\nRequirements (source of truth for "expected"): ' + c.refs + '. Also read CLAUDE.md.',
  ARCH,
...
function commitPrompt(c, clean, rounds) { return [
  ...
  '2. If changed: run ' + BASE + '. If red, do NOT commit (committed=false, changed=true, explain). If green: git add -A && ONE commit, subject "harden(' + c.key + '): <summary>", body, trailer EXACTLY: Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>. Do NOT push.',
