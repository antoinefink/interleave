...
- `CLAUDE.md` — the engineering charter: architecture invariants, layering, Definition of Done.
...
- **Architecture invariants** (enforced by the reviewer; see `CLAUDE.md`): the renderer never
...
the trailer `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>`. The committer
...
function builderPrompt(t) { return [
  ...
  'Read CLAUDE.md, ' + REPO + '/' + t.specFile + ' (the ' + t.id + ' section is your AUTHORITATIVE spec), the ' + t.id + ' roadmap line, and: ' + t.extraDocs,
  ARCH,
...
function reviewerPrompt(t, report) { return [
  ...
  'Read the ' + t.id + ' section of ' + REPO + '/' + t.specFile + ', CLAUDE.md, the roadmap line' + (t.ui ? ', and the design kit references in: ' + t.extraDocs : '') + '.',
  ARCH,
  'Builder report:\n<<<\n' + report + '\n>>>',
...
function committerPrompt(t, clean, rounds) { return [
  ...
  '3. If green: git add -A && ONE commit, subject "' + t.commitMsg + '", body, then trailer EXACTLY: Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>. Do NOT push.',
