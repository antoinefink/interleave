...
export interface SchedulerSignals {
   ...
   * (resolve via T124). NOT T090 calendar-staleness — `KnowledgeStaleness.needsReverify`
   * is the topic-knowledge open-task COUNT, a different shape on a different surface.
   * Optional like `yield`/`retirementSuggestion`: the inspector read model always sets it;
   * display-only adapters that build this shape may omit it (rendered as not-stale).
   */
...
export interface QueueSchedulerSignals {
   ...
   * (resolve via T124). NOT T090 calendar-staleness (`KnowledgeStaleness.needsReverify`
   * is the topic-knowledge open-task COUNT).
   */
  readonly needsReverify: boolean;
}
...
export interface QueueItemSummary {
  ...
  /**
   * The owning SOURCE id of the linked element (T129) — for a `reread_region` task the linked
   * element is the ancestor EXTRACT, so this resolves its source so the open affordance can
   * route to `/source/$id?reread=…`. `null` for non-task / unsourced rows.
   */
