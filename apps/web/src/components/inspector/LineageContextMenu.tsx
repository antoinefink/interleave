...
interface CtxToast {
  ...
  readonly icon: "link" | "copy" | "trash" | "checkCircle";
}
...
export function LineageContextMenu({
  ...
  // The hidden LineageDeleteMenu is driven by a target + a bumped signal (NOT a click).
  ...
  // The cursor position the (driven) delete popover anchors at — the last menu position.
  const [deletePosition, setDeletePosition] = useState<ContextMenuPosition>({ x: 0, y: 0 });
  ...
  const runMutation = useCallback(
    async (label: string, fn: () => Promise<void>) => {
      ...
      try {
        ...
        setToast({ message: e instanceof Error ? e.message : `Could not ${label}`, icon: "trash" });
        onClose();
      } finally {
  ...
  const commitRename = useCallback(
    (node: LineageNode, value: string) => {
      ...
      void runMutation("rename", async () => {
        await appApi.renameElement({ id: node.id, title: next });
      });
    },
    [runMutation],
  );
  ...
  const handlers: LineageNodeMenuHandlers = {
    ...
    copyReference: (node) =>
      copyToClipboard(elementDeepLink(node.id), { message: "Reference copied", icon: "link" }),
    copyText: (node) =>
      copyToClipboard(node.title, { message: "Copied to clipboard", icon: "copy" }),
    setPriority: (node, priority: PriorityLabel) =>
      void runMutation("set priority", async () => {
        await appApi.setElementPriority({ id: node.id, action: { kind: "set", priority } });
      }),
    // Rename opens the inline editor at the node's position (no IPC yet — commit does it).
    rename: (node) => {
      setRename({ node, position: target?.position ?? { x: 0, y: 0 } });
      onClose();
    },
    delete: (node) => {
      // R4: route through the SAME LineageDeleteMenu flow — set the target + bump the
      // signal so its countDescendants → fast-path-or-intent-popover runs. We never call
      // softDeleteSubtree directly here.
      setDeletePosition(target?.position ?? { x: 0, y: 0 });
      setDeleteTarget({ id: node.id, type: node.type, title: node.title });
      setDeleteSignal((n) => n + 1);
      onClose();
    },
    advanceStage: (node) =>
      void runMutation("advance stage", async () => {
        await appApi.updateExtractStage({ id: node.id });
      }),
    ...
    postpone: (node) =>
      void runMutation("postpone", async () => {
        await appApi.postponeExtract({ id: node.id });
      }),
    markDone: (node) =>
      void runMutation("mark done", async () => {
        await appApi.markExtractDone({ id: node.id });
      }),
    suspend: (node) =>
      void runMutation("suspend", async () => {
        await appApi.suspendCard({ cardId: node.id });
      }),
    flagLeech: (node) =>
      void runMutation("flag leech", async () => {
        await appApi.markLeechCard({ cardId: node.id, leech: true });
      }),
    retire: (node) =>
      void runMutation("retire", async () => {
        await appApi.retireCard({ cardId: node.id });
      }),
    restore: (node) =>
      void runMutation("restore", async () => {
        await appApi.restoreFromTrash({ id: node.id });
      }),
    restoreAncestorChain: (node) =>
      void runMutation("restore chain", async () => {
        await appApi.restoreAncestorChain({ id: node.id });
      }),
    purge: (node) =>
      void runMutation("delete permanently", async () => {
        const res = await appApi.purgeFromTrash({ id: node.id });
        if (res.blocked) {
  ...
  const items = target ? buildLineageNodeMenu(target.node, handlers) : [];
  ...
  return (
    <>
       ...
       */}
      <span className="lctx-delete-host" style={{ left: deletePosition.x, top: deletePosition.y }}>
        <LineageDeleteMenu
          target={deleteTarget}
          actions={actions}
          busy={busy}
